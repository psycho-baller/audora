...
export default function ImportConversationPage() {
  ...
  const speedUpAudio = async (file: File, speed = 2): Promise<File> => {
    // Create an AudioContext to decode audio
    const audioContext = new AudioContext();
    const arrayBuffer = await file.arrayBuffer();
    const audioBuffer = await audioContext.decodeAudioData(arrayBuffer);

    // Prepare an OfflineAudioContext to render the sped-up version
    ...
    // Create a buffer source, set playback speed, and render
    ...
    // Convert rendered buffer → WAV blob (reuse your helper)
    const wavBlob = await audioBufferToWav(renderedBuffer);

    // Wrap blob into a File so TypeScript treats it like your original input
    const spedUpFile = new File([wavBlob], file.name.replace(/\.[^/.]+$/, '') + `_2x.wav`, {
      type: 'audio/wav',
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Step 0: Ruduce audio file size
      toast.info('Reducing audio file size...');
      const spedUpAudio = await speedUpAudio(selectedFile, 2);
      ...
      try {
        audioChunks = await splitAudioIntoChunks(spedUpAudio, 10);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Upload an audio file and connect it to a friend
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Upload an audio file (MP3, WAV, M4A, WEBM - max 100MB) or a text transcript (.txt
                  - max 10MB)
                </p>
              </div>
            </div>
          ...
          <div className="bg-card border border-border rounded-xl p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Who was in this conversation with you?
                </p>
              </div>
            </div>
            ...
            {contacts.length === 0 ? (
              <div className="text-center py-8">
                ...
                <Button variant="outline" onClick={() => navigate('/dashboard/network')}>
                  View Network
                </Button>
              </div>
            ) : (
